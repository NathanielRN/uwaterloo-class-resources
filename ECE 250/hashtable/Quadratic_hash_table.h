...
void Quadratic_hash_table<Type>::insert(Type const &obj) {
    ...
    // To speed up our insert, we track the first index where the value can go, which initially assumes
    // that there is no valid index until our probe finds one
	...
	for (int i = 0; i < array_size; i++) {
		...
		bin += i;
		if (occupied[bin & mask] == UNOCCUPIED) {
            // If the first index hasn't been set, we update for the actual insert after the end of this loop
            if (firstValidIndex == -1) {
                firstValidIndex = bin & mask;
            }
            ...
            // If the first valid index hasn't been set, we update it so that the insert knows where to place it
            if (firstValidIndex == -1) {
                firstValidIndex = bin & mask;
            }
        }
	...
	// We managed to finish the for loop so there must be a place to put the incoming value
